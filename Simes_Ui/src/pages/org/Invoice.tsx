...
import { mockInvoiceEstimate } from '@/lib/mock-data';
...
type TariffGroupPreset = {
  hours: { hp: string; peak: string };
  plans: Record<string, TariffPlan>;
};
...
const TARIFF_PRESETS: Record<string, TariffGroupPreset> = {
  D: {
    ...
    plans: {
      ...
      D3: { label: 'D3 Spécial', hpRate: 160, peakRate: 160, monthlyRedevance: 8538, primePerKw: 2882 },
    },
  },
  E: {
    hours: { hp: '00:00-17:00', peak: '17:00-24:00' },
    plans: {
      ...
      E3: { label: 'E3 Spécial', hpRate: 160, peakRate: 160, monthlyRedevance: 8538, primePerKw: 5903 },
    },
  },
  G: {
...
export default function Invoice() {
  ...
  const invoice = mockInvoiceEstimate;
  ...
  // ── API-backed facture computation ─────────────────────
  ...
  // Use API result if available, else latest stored, else mock
  ...
  // Mock-based local estimation (fallback)
  const hpKwh = invoice.totalKwh - invoice.peakKwh;
  const peakKwh = invoice.peakKwh;

  const energyCost = useMemo(() => {
    const hp = Number(hpRate) || 0;
    const peak = Number(peakRate) || 0;
    return hpKwh * hp + peakKwh * peak;
  }, [hpKwh, peakKwh, hpRate, peakRate]);

  const demandCost = useMemo(() => {
    const ps = Number(subscribedPower) || 0;
    const prime = Number(primePerKw) || 0;
    return ps * prime;
  }, [subscribedPower, primePerKw]);

  const fixedCost = Number(monthlyRedevance) || 0;
  const estimatedTotal = energyCost + fixedCost + demandCost;

  const planLabel = TARIFF_PRESETS[tariffGroup].plans[tariffPlan]?.label ?? tariffPlan;

  // Extract API breakdown if present
  ...
  // Displayed values: prefer API, else mock
  const displayTotal = hasLiveResult ? apiTotal : estimatedTotal;
  const displayKwh = hasLiveResult ? apiTotalKwh : invoice.totalKwh;
  const displayMaxDemand = hasLiveResult ? apiMaxDemand : invoice.maxDemandKw;
    ...
    <div className="space-y-6">
      ...
      {hasLiveResult && (
        <div className="rounded-lg border border-primary/30 bg-primary/5 px-4 py-3 text-sm flex items-center gap-3">
          ...
          <div>
            Facture calculée via l'API à partir des données réelles.
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-stagger-children">
        <KpiCard label="Total estimé" value={(displayTotal / 1000000).toFixed(1) + 'M'} unit="XOF" icon={<Receipt className="w-4 h-4" />} />
        <KpiCard label="Consommation" value={(displayKwh / 1000).toFixed(1)} unit="MWh" icon={<Zap className="w-4 h-4" />} />
        <KpiCard label="Puissance max" value={displayMaxDemand.toFixed(1)} unit="kW" icon={<TrendingUp className="w-4 h-4" />} />
      ...
      <div className="rounded-lg border border-severity-warning/30 bg-severity-warning-bg/40 px-4 py-3 text-sm text-severity-warning-foreground flex items-start gap-3">
        ...
        <div>
          PF (cos φ) &lt; 0.93 peut déclencher des pénalités (non calculées en V1).
        </div>
      </div>
        ...
        <Card className="border-primary/20">
          <CardHeader className="pb-2">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <CardTitle className="text-base font-medium">Facture SONABEL (données réelles)</CardTitle>
      ...
      {/* ── Local estimation (mock / preset) ─────────────── */}
      <Card>
        <CardHeader className="pb-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <CardTitle className="text-base font-medium">
              {hasLiveResult ? 'Estimation locale (comparaison)' : 'Facture estimée'}
            </CardTitle>
            <Badge variant="outline" className="text-xs">{tariffGroup} · {planLabel}</Badge>
          </div>
        </CardHeader>
        <CardContent>
          <table className="data-table">
            ...
            <tbody>
              <tr>
                <td className="font-medium">Heures pleines</td>
                <td className="text-right mono">{formatCurrency(hpKwh)}</td>
                <td className="text-right mono">{formatCurrency(Number(hpRate) || 0)}</td>
                <td className="text-right mono font-medium">{formatCurrency(Math.round(hpKwh * (Number(hpRate) || 0)))}</td>
              </tr>
              <tr>
                <td className="font-medium">Heures de pointe</td>
                <td className="text-right mono">{formatCurrency(peakKwh)}</td>
                <td className="text-right mono">{formatCurrency(Number(peakRate) || 0)}</td>
                <td className="text-right mono font-medium">{formatCurrency(Math.round(peakKwh * (Number(peakRate) || 0)))}</td>
              </tr>
              <tr>
                <td className="font-medium">Redevance mensuelle</td>
                <td className="text-right mono">-</td>
                <td className="text-right mono">{formatCurrency(Number(monthlyRedevance) || 0)}</td>
                <td className="text-right mono font-medium">{formatCurrency(Number(monthlyRedevance) || 0)}</td>
              </tr>
              <tr>
                <td className="font-medium">Prime de puissance (PS × prime)</td>
                <td className="text-right mono">{formatCurrency(Number(subscribedPower) || 0)}</td>
                <td className="text-right mono">{formatCurrency(Number(primePerKw) || 0)}</td>
                <td className="text-right mono font-medium">{formatCurrency(Math.round(demandCost))}</td>
              </tr>
              <tr>
                <td className="font-medium">K (taxes/fees) - TBD</td>
                <td className="text-right mono">-</td>
                <td className="text-right mono">-</td>
                <td className="text-right mono font-medium">-</td>
              </tr>
              <tr className="bg-muted/30 font-semibold">
                <td>Total estimé (hors K)</td>
                <td className="text-right mono">{formatCurrency(invoice.totalKwh)}</td>
                <td></td>
                <td className="text-right mono">{formatCurrency(Math.round(estimatedTotal))}</td>
              </tr>
            </tbody>
          </table>

          <div className="mt-4 p-3 rounded-lg bg-muted/30 border text-xs text-muted-foreground">
            Heures de pointe: <span className="mono text-foreground">{peakHours}</span> | Heures pleines: <span className="mono text-foreground">{offPeakHours}</span>
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                <Label>Heures pleines (HP)</Label>
                <Input value={offPeakHours} onChange={(e) => setOffPeakHours(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Heures de pointe (HPointe)</Label>
